...
function EasyBid:OnInitialize()
    ...
    AceEvent:RegisterEvent("CHAT_MSG_SAY", EasyBid:OnMessage)
    AceEvent:RegisterEvent("CHAT_MSG_RAID", EasyBid:OnMessage)
    AceEvent:RegisterEvent("CHAT_MSG_RAID_WARNING", EasyBid:OnMessage)

    AceEvent:RegisterEvent("GET_ITEM_INFO_RECEIVED", EasyBid:OnItemInfoReceived)

    AceComm:RegisterComm("MonDKPDelUsers", EasyBid:OnCommReceived())      -- Broadcasts deleted users (archived users not on the DKP table)
    AceComm:RegisterComm("MonDKPMerge", EasyBid:OnCommReceived())      -- Broadcasts 2 weeks of data from officers (for merging)
    -- Normal broadcast Prefixs
    AceComm:RegisterComm("MonDKPDecay", EasyBid:OnCommReceived())        -- Broadcasts a weekly decay adjustment
    AceComm:RegisterComm("MonDKPBCastMsg", EasyBid:OnCommReceived())      -- broadcasts a message that is printed as is
    AceComm:RegisterComm("MonDKPCommand", EasyBid:OnCommReceived())      -- broadcasts a command (ex. timers, bid timers, stop all timers etc.)
    AceComm:RegisterComm("MonDKPLootDist", EasyBid:OnCommReceived())      -- broadcasts individual loot award to loot table
    AceComm:RegisterComm("MonDKPDelLoot", EasyBid:OnCommReceived())      -- broadcasts deleted loot award entries
    AceComm:RegisterComm("MonDKPDelSync", EasyBid:OnCommReceived())      -- broadcasts deleated DKP history entries
    AceComm:RegisterComm("MonDKPDKPDist", EasyBid:OnCommReceived())      -- broadcasts individual DKP award to DKP history table
    AceComm:RegisterComm("MonDKPMinBid", EasyBid:OnCommReceived())      -- broadcasts minimum dkp values (set in Options tab or custom values in bid window)
    AceComm:RegisterComm("MonDKPMaxBid", EasyBid:OnCommReceived())      -- broadcasts maximum dkp values (set in Options tab or custom values in bid window)
    AceComm:RegisterComm("MonDKPWhitelist", EasyBid:OnCommReceived())      -- broadcasts whitelist
    AceComm:RegisterComm("MonDKPDKPModes", EasyBid:OnCommReceived())      -- broadcasts DKP Mode settings
    AceComm:RegisterComm("MonDKPStand", EasyBid:OnCommReceived())        -- broadcasts standby list
    AceComm:RegisterComm("MonDKPRaidTime", EasyBid:OnCommReceived())      -- broadcasts Raid Timer Commands
    AceComm:RegisterComm("MonDKPZSumBank", EasyBid:OnCommReceived())    -- broadcasts ZeroSum Bank
    AceComm:RegisterComm("MonDKPQuery", EasyBid:OnCommReceived())        -- Querys guild for spec/role data
    AceComm:RegisterComm("MonDKPBuild", EasyBid:OnCommReceived())        -- broadcasts Addon build number to inform others an update is available.
    AceComm:RegisterComm("MonDKPBossLoot", EasyBid:OnCommReceived())      -- broadcast current loot table
    AceComm:RegisterComm("MonDKPBidShare", EasyBid:OnCommReceived())      -- broadcast accepted bids
    AceComm:RegisterComm("MonDKPBidder", EasyBid:OnCommReceived())      -- Submit bids
    AceComm:RegisterComm("MonDKPAllTabs", EasyBid:OnCommReceived())      -- Full table broadcast

    --    EasyBid:SendCommMessage("MonDKPQuery", "start", "GUILD")

    GuildRoster()

    AceConsole:Print("EASY BID INITIALIZED")
end
...
function EasyBid:FillCurrentItem()
    ...
    EasyBid:Print(tostring(EasyBid.var.currentItem));
    EasyBid:Print(tostring(itemLink));
    EasyBid:Print(tostring(itemIcon));
...
function EasyBid:FillBidders()
    ...
    for index, value in ipairs(EasyBid.var.bidders) do
        if (EasyBid.var.gui.scroll ~= nil) then
            ...
            a:SetText(value.dkp .. " " .. value.player)
            EasyBid.var.gui.scroll:AddChild(a)
        end

        if (value.dkp > EasyBid.var.maxBidValue) then
            EasyBid.var.maxBidder = value
            EasyBid.var.maxBidValue = value.dkp
        end
    end

    if (EasyBid.var.gui.highestBidder ~= nil) then
        local highestBidderText = " - "
        if (EasyBid.var.maxBidder ~= nil and EasyBid.var.maxBidder.dkp > 0 and EasyBid.var.maxBidder.player ~= nil) then
            highestBidderText = EasyBid.var.maxBidder.dkp .. " - " .. EasyBid.var.maxBidder.player
        else
            highestBidderText = "0"
        end
...
function EasyBid:OnMessage(self, message, author)
    ...
    elseif(string.find(message, EasyBid.var.bidStartMessage) ~= nil) then
        -- Bid message


        -- Check if item is in the message, if not its invalid. Quit
        local item, itemLink = EasyBid:GetItemFromMessage(message)
        if(item == nil) then
            return
        end

        -- Check if item is in the bid list
        if(EasyBid.var.bidItems[item] == nil) then
            -- Create record of the new item
            EasyBid.var.bidItems[item] = {
                bidCurrent = EasyBid.var.bidStart,
                bidCount = 0,
                bidHolder = nil,
                closed=false
            }
        ...
        local bidItem = EasyBid.var.bidItems[item]

        -- Set this item to current
        EasyBid.var.bidCurrentItem = item

        -- Update GUI
        EasyBid.var.gui.frame:SetStatusText(itemLink)
        EasyBid.var.gui.frame:Show()
        EasyBid.var.gui.btnBid:SetDisabled(false)
        EasyBid:UpdateBidButton()

    elseif(string.find(message, EasyBid.var.bidMessage) ~= nil) then
        ...
        -- Get item
        local bidItem = EasyBid.var.bidItems[EasyBid.var.bidCurrentItem]
        if(bidItem == nil) then
            -- There is no bidding ongoing for any item. return
            EasyBid:Print("No biditem")
            return
        end
        ...
        if(bidItem.bidCount == 0 and isSelf) then
            ...
            minBid = bidItem.bidCurrent +  EasyBid.var.bidMinimumIncrease
        end
        ...
        if(playerName .. "-" .. realmName == author) then
            EasyBid.var.gui.btnBid:SetDisabled(true)
        else
            EasyBid.var.gui.btnBid:SetDisabled(false)
        end
    ...
    elseif(string.find(message, EasyBid.var.bidEndMessage) ~= nil) then
        EasyBid.var.bidItems[EasyBid.var.bidCurrentItem].closed = true
        local function ClearClosed()
            EasyBid:ClearClosedItem(EasyBid.var.bidCurrentItem)
        end
        EasyBid:ScheduleTimer(ClearClosed, EasyBid.var.bidResetDelay)
...
function EasyBid:SetNextMinimum()
    ...
    if (EasyBid.var.maxBidder ~= nil) then
        minimum = minimum + EasyBid.var.maxBidder.dkp
    end
    EasyBid.var.nextMinimum = minimum

    if (EasyBid.var.myBid < minimum and EasyBid.var.gui.editBox ~= nil) then
...
function EasyBid:StartGUI()
    ...
    currentItem:SetPoint("TOP", frame.frame, "TOP", 0, -50);

    local highestBidder = AceGUI:Create("Label")
    highestBidder:SetWidth(250)
    highestBidder:SetHeight(50)
    highestBidder:SetPoint("TOPLEFT", 0, -50);
...
--                return
--        end
--       TODO smazat comment if EasyBid:ValidateSender(sender) then    -- validates sender as an officer. fail-safe to prevent addon alterations to manipulate DKP table
            if (prefix == "MonDKPCommand") then
                local command, arg1, arg2, arg3, arg4 = strsplit(",", message);
--                TODO smazat comment if sender ~= UnitName("player") then
                ...
                if prefix == "MonDKPLootDist" or prefix == "MonDKPDKPDist" or prefix == "MonDKPDelLoot" or prefix == "MonDKPDelSync" or prefix == "MonDKPMinBid" or prefix == "MonDKPWhitelist"
                    ...
                    if success then
                        ...
                        elseif prefix == "MonDKPBidShare" then
                            EasyBid.var.bidders = deserialized
                            EasyBid:FillBidders()
                            return
...
function EasyBid:SendData(prefix, data, target)
    ...
    if IsInGuild() then
        if prefix == "MonDKPQuery" or prefix == "MonDKPBuild" or prefix == "MonDKPTalents" or prefix == "MonDKPRoles" then
            AceComm:SendCommMessage(prefix, data, "SAY")
            return;
    ...
    if IsInGuild() then
        ...
        if (prefix == "MonDKPZSumBank" or prefix == "MonDKPBossLoot" or prefix == "MonDKPBidShare") then    -- Zero Sum bank/loot table/bid table data and bid submissions. Keep to raid.
            AceComm:SendCommMessage(prefix, packet, "SAY")
            return;
        end
    end
end
